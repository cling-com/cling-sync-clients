...
extension ClingSyncUITests {
    ...
    // A single tap lands the cursor wherever the center hits (or selects a word the
    // spell checker flags), so backwards deletes can leave text behind. A triple tap
    // selects the whole line, which the typed value then replaces.
